	...
	while (1) {
		...
		if (eyeGazes.gaze[XR_EYE_POSITION_LEFT_FB].isValid) {
			...
			std::cout << "gaze:"
				...
				<< dataLeftYaw << ","
				<< dataRightPitch << ","
				<< dataRightYaw << std::endl;
		}
		else {
			std::cout << "gaze:invalid" << std::endl;
		}
		//Face
		...
		if (expressionWeights.status.isValid) {
			...
			std::cout << "face:"
				<< dataEyesClosedAmountL << ","
				<< dataEyesClosedAmountR << std::endl;
		}
		else {
		...
		p << osc::BeginMessage("/tracking/eye/EyesClosedAmount")
